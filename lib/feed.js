...
function Feed(options) {
    ...
    if(options.webfeeds) {
      this.webfeeds = {};
      this.webfeeds.accentColor = options.webfeeds.accentColor;
      this.webfeeds.analytics = options.webfeeds.analytics; // Google Analytics tracking ID
      this.webfeeds.cover = options.webfeeds.cover;
      this.webfeeds.icon = options.webfeeds.icon;
      this.webfeeds.logo = options.webfeeds.logo;
    }
...
function rss_2_0(options) {
    ...
    if(options.webfeeds && options.webfeeds.cover)
      channel.push({'webfeeds:cover': { _attr: {image: options.webfeeds.cover}}});

    if(options.webfeeds && options.webfeeds.icon)
      channel.push({'webfeeds:icon': options.webfeeds.icon});

    if(options.webfeeds && options.webfeeds.logo)
      channel.push({'webfeeds:logo': options.webfeeds.logo});

    if(options.webfeeds && options.webfeeds.accentColor)
      channel.push({'webfeeds:accentColor': options.webfeeds.accentColor});

    if(options.webfeeds && options.webfeeds.analytics)
      channel.push({'webfeeds:analytics': { _attr: {id: options.webfeeds.analytics, engine: 'GoogleAnalytics'}}});
    ...
    if(options.webfeeds)
        container[0].rss[0]._attr['xmlns:webfeeds'] = 'http://webfeeds.org/rss/1.0';
